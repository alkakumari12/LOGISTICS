...
<style>
  textarea{
    ...
    /* Style for card header and body */
    .card-header,
    .card-body {
            ...
            <div class="form-group" id="tbi-field">
              <label for="" class="control-label">Pickup Branch</label>
                <select name="to_branch_id" id="to_branch_id" class="form-control select2">
                  <option value=""></option>
             <?php 
        // Existing code to fetch branches from the database
        while ($row = $branches->fetch_assoc()): ?>
...
<script>
  ...
  $('#manage-parcel').submit(function(e){
    ...
    if($('#parcel-items tbody tr').length <= 0){
      alert_toast("Please add at least 1 parcel information.","error"); // Make sure alert_toast function is defined
      end_load();
      return false;
    }
    $.ajax({
  ...
  error: function(xhr, status, error) {
    ...
    alert_toast('An error occurred while saving data. Please try again later.', "error");
  }
});
...
$(document).ready(function() {
    ...
    $('[name="weight[]"], [name="height[]"]').on('input', function() {
        // Get the weight and height values
        var weight = parseFloat($(this).closest('tr').find('[name="weight[]"]').val());
        var height = parseFloat($(this).closest('tr').find('[name="height[]"]').val());

        // Perform some calculation to determine the price
        var price = calculatePrice(weight, height); // Implement your logic here to calculate price

        // Set the calculated price in the price field
        $(this).closest('tr').find('[name="price[]"]').val(price.toFixed(2)); // Assuming price is calculated and set with 2 decimal places
    });
});

// Function to calculate price based on weight and height
function calculatePrice(weight, height) {
    // Implement your logic to calculate price based on weight and height
    // For example:
    // Calculate price as the product of weight and height
    var price = weight * height;
    return price;
}
</script>